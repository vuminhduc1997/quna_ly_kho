...
<div class="row">
    ...
    <div style="margin-left:-1px"  class="span13">
                                ...
                                <div id="acct-password-row" class="span8">
                                    <fieldset>
                                        ...
                                        <div class="control-group">
                                            ...
                                            <textarea name="txtLyDo" id="input" class="span7" rows="3" required="required"  disabled="true">{{ $chuyenkho->ck_lydo }}</textarea>
                                        </div>
                                    </fieldset>
                                </div>
                                <div id="acct-password-row" class="span4">
                                    <fieldset>
                                        ...
                                        <div class="control-group">
                                            ...
                                            <input type="date" name="txtDate" value="{!! $chuyenkho->ck_ngay !!}" class="span3" disabled="true">
                                        </div>
                                        <div>
                                        ...
                                        <table class="tb table table-bordered table-hover" id="myTable" name="myTable">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    ?>
                                                        <td>{!! $item->vt_id !!}
                                                        <input type="hidden"  class="vtID" name="" value="{!! $item->vt_id !!}">
                                                        <input type="hidden"  class="ckID" name="" value="{!! $item->ck_id !!}">
                                                        </td>
                                                        <td>{!! $vt->vt_ten !!}</td>
                                                        <td>{!! $kho1->kho_ten !!}</td>
                                                        <td>{!! $kho2->kho_ten !!}</td>
                                                        <td>{!! $dvt->dvt_ten !!}</td>
                                                        <td>
                                                            <input type="number" name="txtSL" value="{!! $item->ctck_soluong !!}" class="qty span3">
                                                        </td>
                                                        <td>{!! number_format("$vt->vt_gia",0,".",",")  !!} vnđ</td>
                                                        <td>{!! number_format("$item->ctck_thanhtien",0,".",",")  !!} vnđ</td>
                                                        <td>
                                                            <a href="{!! URL::route('chucnang.chuyenkho.getDeletePro',[$item->vt_id,$chuyenkho->id]) !!}">xóa</a>
        ...
        $(document).ready(function() {
            $(".del").click(function(){
                var ckID = $(this).parent().parent().find(".ckID").val();
                var vtID = $(this).parent().parent().find(".vtID").val();
                var qty = $(this).parent().parent().find(".qty").val();
                var token = $("input[name='_token']").val();
                // alert(idKho2);
                $.ajax({
                    url:'http://localhost/quanlykho/chucnang/chuyenkho/suavattu/'+vtID+'/'+qty,
                    type:'GET',
                    cache:false,
                    data:{"_token":token,"ckID":ckID,"qty":qty,"vtID":vtID},
                    success: function(data) {
                        if(data == "oke") {
                          window.location = "http://localhost/quanlykho/chucnang/chuyenkho/sua-theo-vat-tu/"+ckID;
                        }
                        else {
                         alert("Error!");
                        }
